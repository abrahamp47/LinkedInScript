...
def _uninstall(project_root: Path) -> None:
    ...
    uninstall_script = project_root / "scripts" / "uninstall-task.ps1"
    if uninstall_script.exists():
        result = subprocess.run(
            ["powershell", "-ExecutionPolicy", "Bypass", "-File", str(uninstall_script)],
            capture_output=True,
            text=True,
        )
        if result.returncode == 0:
            ...
            print(f"  Could not remove task (may require admin): {result.stderr.strip()}")
    else:
        print("  No install script found — skipping.")
