...
# Re-auth alert markers (written by src/scraper/auth.py)
_DATA_DIR = Path(__file__).resolve().parent.parent.parent / "data"
_REAUTH_MARKER = _DATA_DIR / ".reauth_needed"
_REAUTH_ALERTED = _DATA_DIR / ".reauth_alerted"

REAUTH_ALERT_HTML = """\
<html><body>
<h2 style="color: #d32f2f;">LinkedInScript: LinkedIn Login Required</h2>
<p><strong>The saved LinkedIn session has expired and could not be refreshed
automatically.</strong></p>
<p>Scheduled runs cannot log in to LinkedIn on their own (no browser window is
available during an unattended run), so the tool is currently falling back to
the limited guest API and results may be incomplete or empty.</p>
<h3>What to do (one-time, ~30 seconds):</h3>
<ol>
<li>Open a terminal in the LinkedInScript folder</li>
<li>Run: <code>python main.py</code></li>
<li>A browser window opens — log in to LinkedIn</li>
<li>The window closes automatically and daily runs resume normally</li>
</ol>
</body></html>
"""

REAUTH_ALERT_TEXT = """\
LinkedInScript: LinkedIn Login Required

The saved LinkedIn session has expired and could not be refreshed automatically.

Scheduled runs cannot log in to LinkedIn on their own (no browser window is
available during an unattended run), so the tool is currently falling back to
the limited guest API and results may be incomplete or empty.

What to do (one-time, ~30 seconds):
  1. Open a terminal in the LinkedInScript folder
  2. Run: python main.py
  3. A browser window opens -- log in to LinkedIn
  4. The window closes automatically and daily runs resume normally
"""


def check_reauth_alert(config: dict) -> None:
    """Email a re-auth alert when LinkedIn login is required for unattended runs.

    The scraper's auth layer writes a `.reauth_needed` marker when a scheduled
    run finds the LinkedIn session dead and cannot open an interactive browser.
    This sends a one-time email (deduped via `.reauth_alerted`) telling the user
    to run `python main.py` once to log in. The markers are cleared automatically
    on the next successful authentication.

    Args:
        config: Full config dict (passed to send_email for SMTP settings).
    """
    if not _REAUTH_MARKER.exists():
        # Session is healthy — clear any stale "already alerted" flag
        _REAUTH_ALERTED.unlink(missing_ok=True)
        return

    if _REAUTH_ALERTED.exists():
        # Already emailed for this re-auth event — don't spam every run
        logger.warning("LinkedIn re-auth still needed (alert already sent).")
        return

    subject = "LinkedInScript: LinkedIn login required (action needed)"
    try:
        send_email(config, subject, REAUTH_ALERT_HTML, REAUTH_ALERT_TEXT)
        _REAUTH_ALERTED.write_text("1", encoding="utf-8")
        logger.warning("Re-auth alert email sent — run 'python main.py' to log in.")
    except Exception as e:
        logger.error("Failed to send re-auth alert: %s", e)
