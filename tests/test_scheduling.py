...
class TestReauthAlert:
    """Tests for check_reauth_alert -- LinkedIn re-login required email."""

    @pytest.fixture(autouse=True)
    def _setup_markers(self, tmp_path, monkeypatch):
        """Redirect re-auth marker paths to tmp_path for isolation."""
        self.marker = tmp_path / ".reauth_needed"
        self.alerted = tmp_path / ".reauth_alerted"
        monkeypatch.setattr("src.scheduling.health._REAUTH_MARKER", self.marker)
        monkeypatch.setattr("src.scheduling.health._REAUTH_ALERTED", self.alerted)
    ...
    def test_no_marker_no_alert(self, mock_send_email, sample_config):
        """No re-auth marker -> no email sent."""
        from src.scheduling import check_reauth_alert

        check_reauth_alert(sample_config)
        mock_send_email.assert_not_called()

    def test_marker_present_sends_alert(self, mock_send_email, sample_config):
        """Re-auth marker present -> email sent and 'alerted' flag written."""
        from src.scheduling import check_reauth_alert

        self.marker.write_text("123", encoding="utf-8")
        check_reauth_alert(sample_config)

        mock_send_email.assert_called_once()
        assert self.alerted.exists()

    def test_alert_sent_once_only(self, mock_send_email, sample_config):
        """Second run with marker still present does NOT re-send the email."""
        from src.scheduling import check_reauth_alert
    ...
    def test_alerted_flag_cleared_when_session_healthy(self, mock_send_email, sample_config):
        """When marker disappears (re-auth done), stale 'alerted' flag is cleared."""
        from src.scheduling import check_reauth_alert

        # Simulate a prior alert, then the user logged in (marker gone)
        self.alerted.write_text("1", encoding="utf-8")
        check_reauth_alert(sample_config)

        mock_send_email.assert_not_called()
        assert not self.alerted.exists()

    def test_alert_subject_mentions_login(self, mock_send_email, sample_config):
        """Alert subject clearly signals login is required."""
        from src.scheduling import check_reauth_alert
